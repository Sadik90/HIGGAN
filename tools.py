...
def del_data(inFile):
    ...
    print('shape of new seqeunce：', newseq.shape)
...
#     print(confusion_matrix(y, y_pred).ravel()) 
    ...
    eval_dictionary = {
        ...
        "GMean": geometric_mean_score(y, y_pred, average='binary'), #Gmean
        "SEN": tp / (tp + fn), #Sensitivity
        "PREC": tp / (tp + fp), #Precision
        "SPEC": tn / (tn + fp), #Specificity
        "MCC": matthews_corrcoef(y, y_pred), #MCC
...
def cv(model,df_X,df_y,n_folds=5):
    eval_dict = []
    kf = KFold(n_splits=n_folds, shuffle=True, random_state=10)#K- 5 fold validation。
#     model1 = model
